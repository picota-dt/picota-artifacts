...
def _load_incidencias_lookup(
...
) -> tuple[dict[str, dict[str, str]], list[str], int, int, int, int, int, int]:
    ...
    with incidencias_jsonl.open("r", encoding="utf-8") as source:
        for line_number, raw_line in enumerate(source, start=1):
            ...
            key = _normalize_join_key(row.get("Vale"))
            if not key:
                continue
    ...
    try:
        with output_tsv.open("r", encoding="utf-8", newline="") as source, joined_path.open(
            ...
            for row in reader:
                ...
                join_key = _normalize_join_key(row[albaran_index])
                incidencias_by_code = incidencias_lookup.get(join_key)
                if incidencias_by_code is None:
                    production_rows_without_match += 1
                    writer.writerow(row + ["0"] * len(appended_codes))
                else:
                    production_rows_with_match += 1
                    writer.writerow(row + [incidencias_by_code.get(code, "0") for code in appended_codes])
...
def main() -> int:
    ...
    default_output = default_data_dir / "europlatano.tsv"
    ...
    parser.add_argument(
        ...
        help="Directory with one meteo TSV per station code (e.g. 3BF.tsv).",
    )
    ...
    try:
        ...
        zero_position_fca_codes = _load_zero_position_fca_codes(fincas_tsv=fincas_tsv)
        meteo_columns = ["tmed", "tmin", "tmax", "prec", "hrMedia", "velmedia", "dir", "racha"]
        meteo_rows_direct, meteo_rows_fallback, meteo_rows_incomplete, meteo_rows_blocked_zero_position = (
            _enrich_output_tsv_with_meteo(
                output_tsv=output_tsv,
                meteo_dir=meteo_dir,
                meteo_columns=meteo_columns,
                tmp_dir=args.tmp_dir,
                drop_fca=False,
                blocked_station_codes=zero_position_fca_codes,
            )
        )
        ...
        print(f"Rows with meteo complete from direct station/day: {meteo_rows_direct}")
        print(f"Rows completed with meteo fallback (near stations): {meteo_rows_fallback}")
        print(f"Rows still incomplete after meteo fallback: {meteo_rows_incomplete}")
